...
module TripletexRubyClient
  class BankBalanceEstimation
    ...
    # boolean
    attr_accessor :is_incoming_invoice
    ...
    attr_accessor :vendor_or_customer_name

    # boolean
    attr_accessor :is_manually_added
    ...
    def recurrence=(recurrence)
      validator = EnumAttributeValidator.new('String', ['NONE', 'DAILY', 'WEEKLY', 'MONTHLY'])
      unless validator.valid?(recurrence)
        fail ArgumentError, 'invalid value for "recurrence", must be one of #{validator.allowable_values}.'
      end
      @recurrence = recurrence
    end
    ...
    def category=(category)
      validator = EnumAttributeValidator.new('String', ['DUPLICATE', 'STARTING_BALANCE', 'NONE', 'SALARY', 'ENI', 'TAX', 'VAT_RETURNS'])
    ...
    def batch_id=(batch_id)
      if !batch_id.nil? && batch_id.to_s.length > 255
        fail ArgumentError, 'invalid value for "batch_id", the character length must be smaller than or equal to 255.'
      end
